...
def get_db_connection():
    try:
        conn = mysql.connector.connect(
            ...
            database=os.environ.get('DB_NAME', 'user_auth')
        )
        return conn
...
def signup():
    if request.method == 'POST':
        ...
        if not cursor:
            flash("⚠ Database not available. Signup disabled.")
...
def login():
    if request.method == 'POST':
        ...
        conn, cursor = get_cursor()
        if not cursor:
            flash("⚠ Database not available. Login disabled.")
            return redirect(url_for('login'))
        ...
        try:
            ...
            # Fetch stored password hash and verify
